...
def _auto_populate_derivative_directories(parallel_run_settings):
    ...
    parallel_run_settings['OpenFace_CSV_Path'] = str(pathlib.Path(
        os.path.join(parallel_run_settings['MONAH_Earnings_Call'], 'staging')
    ))

    parallel_run_settings['avi_path'] = str(pathlib.Path(
        os.path.join(parallel_run_settings['MONAH_Earnings_Call'], 'downloaded_videos')))
    parallel_run_settings['wav_path'] = str(pathlib.Path(
        os.path.join(parallel_run_settings['MONAH_Earnings_Call'], 'downloaded_audio')))
    parallel_run_settings['csv_path'] = str(pathlib.Path(
        os.path.join(parallel_run_settings['MONAH_Earnings_Call'], 'output')))

    parallel_run_settings['usb_csv_path'] = str(pathlib.Path(
        os.path.join(parallel_run_settings['MONAH_Earnings_Call'], 'open_face_csv')))
    ...
    parallel_run_settings['test_file_path'] = str(
        os.path.join(parallel_run_settings['MONAH_Earnings_Call'], 'test_files'))
...
def get_parallel_run_settings(computer_name):
    ...
    if computer_name == 'marriane_win':
        parallel_run_settings['MONAH_Earnings_Call'] = str(pathlib.Path("E:/MONAH_EarningsCall"))
    ...
    if computer_name == 'marriane_linux':
        parallel_run_settings['MONAH_Earnings_Call'] = str(pathlib.Path("/datadrive/MONAH"))
    ...
    if computer_name == 'joshua_linux':
        parallel_run_settings['MONAH_Earnings_Call'] = str(pathlib.Path("/mnt/S/MONAH"))
